...
def DescribeOrUpdateIntent(obs_images, prev_intent=None, processor=None, model=None, tokenizer=None, args=None, doScenes_instruction=None):
    """
    Our comment:
        This is where the model decides what the car should do next.
        Additions:
        1. Add passenger instruction to the prompt.
           Ex: "Switch to the left lane to avoid the construction on the right lane, and preceed on the right lane after the construction is clear."
        2. Add a note to the model to always prioritize the passenger's instruction unless it is unsafe.
    """

    if doScenes_instruction:
        passenger_instruction_prompt = f'The passenger says: "{doScenes_instruction}". '
        passenger_importance_note = (
            "Always prioritize the passenger’s instruction unless it is unsafe; "
            "if complying is unsafe, briefly explain why and pick the safest alternative."
        )
    else:
        passenger_instruction_prompt = ""
        passenger_importance_note = ""

    if prev_intent is None:

        if "llava" in args.model_path:
            prompt = passenger_instruction_prompt + f"""You are a autonomous driving labeller. 
            You have access to a front-view camera images of a vehicle taken at a 0.5 second interval over the past 5 seconds.
            Imagine you are driving the car. Based on the lane markings and the movement of other cars and pedestrians, provide a concise description of the desired intent of the ego car. 
            Is it going to follow the lane to turn left, turn right, or go straight?
            Should it maintain the current speed or slow down or speed up? """ + passenger_importance_note
        
    else:
        prompt = passenger_instruction_prompt + f"""You are a autonomous driving labeller. 
        You have access to a front-view camera images of a vehicle taken at a 0.5 second interval over the past 5 seconds. 
        Imagine you are driving the car. Half a second ago your intent was to {prev_intent}. 
        Based on the updated lane markings and the updated movement of other cars and pedestrians, do you keep your intent or do you change it?
        Explain your current intent: """ + passenger_importance_note
        ...
        if "llava" in args.model_path:
            ...
            Imagine you are driving the car. Half a second ago your intent was to {prev_intent}. 
            Based on the updated lane markings and the updated movement of other cars and pedestrians, do you keep your intent or do you change it? 
            Provide a concise description explanation of your current intent: """ + passenger_importance_note
...
def GenerateMotion(obs_images, obs_waypoints, obs_velocities, obs_curvatures, given_intent, processor=None, model=None, tokenizer=None, args=None):
    ...
    if args.method == "openemma":
        ...
        intent_description = DescribeOrUpdateIntent(obs_images, prev_intent=given_intent, processor=processor, model=model, tokenizer=tokenizer, args=args, doScenes_instruction=args.doScenes_annotation,)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model-path", type=str, default="llava")  # default to LLava
    parser.add_argument("--plot", type=bool, default=True)
    parser.add_argument("--dataroot", type=str, default="/mnt/data/nuscenes/v1.0-trainval_meta")
    parser.add_argument("--version", type=str, default="v1.0-trainval")
    parser.add_argument("--method", type=str, default="openemma")
    parser.add_argument("--doScenes", type=str, default="")
    args = parser.parse_args()

    if not os.path.exists(args.dataroot):
        print(f"\n[ERROR] The path to nuScenes dataset does not exist: {args.dataroot}")
        print("          Please check if the nuScenes dataset folder was moved or renamed.\n")
    ...
    # Ask the user for the scene number
    scene_input = input("Enter scene number to view (e.g., '3' for scene-0003): ").strip()
    desired_scene = "scene-" + scene_input.zfill(4)
    print("You selected:", desired_scene)

    if args.doScenes:
        doScenes_annotation = args.doScenes
    else:
        doScenes_annotation = input("Enter your doScenes annotation (or press Enter to skip): ").strip()
        if not doScenes_annotation:
            print("***No doScenes annotation provided. Continuing without injection.** ")
    args.doScenes_annotation = doScenes_annotation
    ...
    for scene in scenes:
        ...
        for i in range(scene_length - TTL_LEN):
            ...
            for rho in range(3):
                ...
                pred_waypoints = prediction.replace("Futurs and curvatures:", "").strip()
                coordinates = re.findall(r"\[([-+]?\d*\.?\d+),\se speed*([-+]?\d*\.?\d+)\]", pred_waypoints)
                if not coordinates == []:
                    break
            if coordinates == []:
                continue
